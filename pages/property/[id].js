...
const PropertyDetails = ({ propertyDetails: { price, rentFrequency, rooms, title, baths, area, agency, isVerified, description, type, purpose, furnishingStatus, amenities, photos } }) => {
  ...
  {photos && <Scrollbar data={photos} />}
    <Box w='full' p='6 0'>
      ...
      <Flex className='property_heading_text' alignItems='center' pt='2' justifyContent='space-between' w='350px' color='blue.400'>
        {rooms}<FaBed /> | {baths} <FaBath /> | {millify(area)} sqft <BsGridFill />
      </Flex>
    </Box>
    <Box marginTop='2'>
      <Text fontSize='lg' marginBottom='2' fontWeight='bold'>{title}</Text>
      <Text className='property_text' lineHeight='2' color='gray.600'>{description}</Text>
    </Box>
